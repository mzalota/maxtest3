...
def list(request):
    ...
    return render_to_response('studio/studios.html',
                            { 'studios': studios, },
                            context_instance=RequestContext(request))

def profile(request, studio_url_name):
...
def schedule(request, studio_url_name):
    studios = Studio.objects.filter(name_url=studio_url_name)
    studio = studios[0]
...
def facebookFeed(request, studio_url_name):
