...
def run():
    ...
    scraper = Scraper()
    studios = Studio.objects.all().filter(id__gte=21).filter(id__lte=200).order_by('id')
...
def process_studio(scraper, studio):
    ...
    if not studio_site_set:
        logger.warn("Config data is not specified for studio: "+str(studio.id)+", "+studio.name)
        return
    ...
    for step in config_parse:
        ...
        events = parseMBO(scraper).run(studio, step['headers'], filters)
        if not events:
            logger.warn("Did not parse any events from HTML")
            continue

        logger.info("Saving " + str(len(events)) + " events to DB for studio: "+studio.name)
        for event in events:
            event.save()
